...
import wakfulib.ui.tv.porst.splib.convert.ConvertHelpers;
import wakfulib.ui.tv.porst.splib.gui.GuiHelpers;
import wakfulib.ui.tv.porst.splib.gui.caret.ICaretListener;
import wakfulib.ui.tv.porst.splib.gui.caret.JCaret;
...
public final class JHexView extends JComponent {
	...
	private final ArrayList<IHexViewHoverListener> m_hoverListeners = new ArrayList<>();
	 ...
	 */
	private Font m_font = new Font(GuiHelpers.getMonospaceFont(), Font.PLAIN, 20);
	 ...
	 */
	private Color m_bgColorOffset = Color.LIGHT_GRAY;
	 ...
	 */
	private final Color m_disabledColor = RGB.newColorWithAlpha(Color.GRAY, 0.5);
	 ...
	 */
	private final Color m_colorHighlight = new Color(231, 230, 230);
	...
	private void changeBy(ActionEvent event, final int length) {
				...
				if (isEnabled()) {
					...
					final long normalizedOffset = m_flipBytes ?
							...
							if (isSelectedOffset(normalizedOffset)) {
								...
								g.setColor(color == null ? oldColor : color);
							}
	 ...
	 */
	private void drawCaretAsciiWindow(Graphics g, final int characterWidth, final int characterHeight) {
	 ...
	 * @param characterHeight The height of a single character.
	 */
	private void drawCaretHexWindow(Graphics g, final int characterWidth, final int characterHeight) {
	...
	private void drawMouseOverHighlighting(Graphics g) {
		...
		if (m_lastHighlightedNibble == -1) {
			if (lastNibbleSent != -1) {
				for (IHexViewHoverListener m_hoverListener : m_hoverListeners) {
					m_hoverListener.hoverRemoved();
				}
				lastNibbleSent = -1;
			}
			return;
		...
		if (lastNibbleSent != m_lastHighlightedNibble) {
			for (IHexViewHoverListener m_hoverListener : m_hoverListeners) {
				m_hoverListener.hoverChanged(m_lastHighlightedNibble);
			}
		}
		...
		lastNibbleSent = m_lastHighlightedNibble;
	...
	private ColoredRange findColoredRange(long currentOffset) {
		...
		for (ColoredRangeManager element : m_coloredRanges) {
	...
	private Rectangle getByteBoundsAscii(int position) {
		...
		return new Rectangle(x, y, m_charWidth, m_charHeight + 2);
	}
	 ...
	 */
	private int getNibbleAtCoordinate(int x, final int y) {
	 ...
	 * nibble at the coordinates.
	 */
	private int getNibbleAtCoordinatesAscii(int x, final int y) {
	 ...
	 * nibble at the coordinates.
	 */
	private int getNibbleAtCoordinatesHex(int x, final int y) {
	...
	private boolean isInsideAsciiView(int x, final int y) {
		return y >= m_paddingTop - m_font.getSize() && x >= getAsciiViewLeft();
	}

	private boolean isInsideHexView(int x, final int y) {
	...
	public boolean isSelectedOffset(long currentOffset) {
	...
	private void setCurrentPosition(long newPosition) {
		...
		for (IHexViewListener listener : m_listeners) {
	...
	protected void paintComponent(Graphics gx) {
        ...
        if (gx instanceof Graphics2D g2d) {
	...
	public void addHoverListener(IHexViewHoverListener listener) {
		if (listener == null) {
			throw new NullPointerException("Error: Listener can't be null");
		}

		// Avoid duplicates
		if (!m_hoverListeners.contains(listener)) {
			m_hoverListeners.add(listener);
		}
	}
	...
	public void removeHexHoverListener(IHexViewHoverListener listener) {
		...
		m_hoverListeners.remove(listener);
	}
	 ...
	 */
	public void colorize(int level, final long offset, final int size, final Color color, final Color bgcolor) {
	 ...
	 * @throws IllegalArgumentException Thrown if offset is negative or size is not positive.
	 */
	public void uncolorize(int level, final long offset, final int size) {
	...
	public void uncolorizeAll() {
		...
		for (ColoredRangeManager coloredRange : m_coloredRanges) {
			coloredRange.clear();
		}
	}
	...
	private class InternalListener implements AdjustmentListener, MouseListener, MouseMotionListener, FocusListener, ICaretListener, IDataChangedListener, ComponentListener, KeyListener, MouseWheelListener {
		...
		private void keyPressedInAsciiView(KeyEvent arg0) {
            if (ConvertHelpers.isHexCharacter(arg0.getKeyChar())) {
...
//                m_dataProvider.setData();
            }
		}

		private void keyPressedInHexView(KeyEvent key) {

			int selectionLength = (int) Math.max(1, getSelectionLength() / 2);

			final byte[] data = m_dataProvider.getData(getCurrentOffset(), selectionLength);
			...
			int pos = (int)m_baseAddress;
			if (getSelectionLength() > 1) {
				for (int i = 0; i < getSelectionLength() * 2; i++) {
					if ((getSelectionStart() + i) % 2 == 0) {
						data[pos + i] = (byte) (data[pos + i] & 0x0F | value << 4);
					} else {
						data[pos + i - 1] = (byte) (data[pos + i - 1] & 0xF0 | value);
					}
				}
			} else {
				if (getSelectionStart() % 2 == 0) {
					data[pos] = (byte) (data[pos] & 0x0F | value << 4);
				} else {
					data[pos] = (byte) (data[pos] & 0xF0 | value);
				}
			}
		...
		public void keyPressed(KeyEvent arg0) {
			...
			if (!isEnabled() || !editable || ! m_dataProvider.isEditable()) {
				return;
			}
...
//				if (m_dataProvider.isEditable() && ConvertHelpers.isHexCharacter(arg0.getKeyChar())) {
	...
	private class WaitingForDataAction extends AbstractAction {
		...
		private WaitingForDataAction(long offset, final int size) {
			m_offset = offset;
			m_size = size;
		}
