...
public class JXCollapsiblePane extends JPanel {
    ...
    /**
     * Constructs a new JXCollapsiblePane with a {@link JPanel} as content pane and the specified
     * direction.
     *
     * @param direction
    ...
    public JXCollapsiblePane(Direction direction) {
        ...
        setAnimationParams(new AnimationParams(30, 8, 1.0f, 1.0f));
    ...
    public void setCollapsed(boolean val) {
        ...
        if (isAnimated() && isShowing()) {
            ...
            if (oldValue) {
                ...
                setAnimationParams(new AnimationParams(30, Math.max(8, preferredDimension / delta), 1.0f, 1.0f));
                animator.reinit(dimension, preferredDimension);
                wrapper.getView().setVisible(true);
            } else {
                int dimension = direction.isVertical() ? wrapper.getHeight() : wrapper.getWidth();
                setAnimationParams(new AnimationParams(30, Math.max(8, dimension / delta), 1.0f, 1.0f));
    ...
    private final class WrapperContainer extends JViewport implements AlphaPaintable {
        ...
        public void paint(Graphics g) {
            ...
            if (getAlpha() == 1f) {
                ...
                throw new IllegalStateException("translucent not supported !");
...
//                }
    ...
    public static JXCollapsiblePane collapsiblePaneWithTitle(Container container, JXCollapsiblePane.Direction direction, String label) {
        JXCollapsiblePane jxCollapsiblePane = new JXCollapsiblePane();
        jxCollapsiblePane.setDelta(2);
        jxCollapsiblePane.setDirection(direction);
        ...
        jxCollapsiblePane.addPropertyChangeListener("collapsed", evt -> {
            String text = title.getText();
            title.setText(text.substring(0, text.length() - 1) + (((Boolean) evt.getNewValue()) ? "+" : "-"));
        });
        container.add(title);
        container.add(jxCollapsiblePane);
